...
int main(int argc, char** argv){
  ...
  if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
     {ROS_INFO("Robot reached Drop-Off Position");
     ros::Duration(5.0).sleep();}
  else
     {ROS_INFO("Robot didn't reach Drop-off location ");
}
    }
  else
    {ROS_INFO("Robot didn't reach Pick-up location ");
}
